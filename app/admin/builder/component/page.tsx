...
export default function ComponentBuilderPage() {
  ...
  const handleAddComponent = (type: ComponentType | string, isCustom?: boolean, customData?: any) => {
    // For component builder, we only allow framework components (text, richtext, enum)
    if (type === COMPONENT_TYPES.TEXT || type === COMPONENT_TYPES.RICHTEXT || type === COMPONENT_TYPES.ENUM) {
      try {
    ...
    <div className="min-h-screen bg-background relative overflow-hidden">
      ...
      <div className="flex h-[calc(100vh-61px)] relative">
        ...
        {sidebarOpen && (
          ...
          >
            <ComponentPalette onAddComponent={handleAddComponent} builderType="component" />
          </motion.div>
        )}
