...
export default function DocumentBuilderPage() {
  ...
  const handleAddComponent = (type: ComponentType | string, isCustom?: boolean, customData?: any) => {
    if (isCustom && customData) {
      // Handle custom component - create field values from default values
      const fieldValues: Record<string, string | boolean> = {};
      customData.fields.forEach((field: any) => {
        if (field.defaultValue) {
          if (field.type === 'checkbox') {
            fieldValues[field.name] = field.defaultValue === 'true' || field.defaultValue === true;
          } else {
      ...
      const newComponent: Component = {
        ...
        data: {
          ...
          value: '',
          customComponentId: customData.id,
    ...
    <div className="min-h-screen bg-background relative overflow-hidden">
      ...
      <div className="flex h-[calc(100vh-61px)] relative">
        ...
        {sidebarOpen && (
          ...
          >
            <ComponentPalette onAddComponent={handleAddComponent} builderType="document" />
          </motion.div>
        )}
