...
import { loadCustomComponents, CustomComponent } from '@/lib/customComponentsStorage';
...
export default function ComponentPalette({ onAddComponent, builderType = 'document' }: ComponentPaletteProps) {
  ...
  const handleAdd = (type: ComponentType | string, isCustom: boolean = false, customData: CustomComponent | null = null) => {
    if (isCustom && customData) {
      onAddComponent(type, true, customData);
    } else {
      onAddComponent(type, false, null);
    }
  };
