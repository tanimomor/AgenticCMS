...
import { generateMockMetrics, generateActivityFeed, generateRecentContent, generatePerformanceData } from '@/lib/mock-data';
        ...
        <main className="p-8 space-y-8">
          ...
          {metrics && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { label: 'Published Today', value: metrics.publishedToday, sublabel: '+12 from yesterday', trend: 'up' },
                { label: 'Views This Month', value: metrics.viewsThisMonth.toLocaleString(), sublabel: `${metrics.engagementRate}% engagement`, trend: 'up' },
                { label: 'Avg. Read Time', value: metrics.averageReadTime, sublabel: 'Across all content', trend: 'stable' },
              ].map((metric, index) => (
                <motion.div
                  key={metric.label}
                  initial={{ opacity: 0, y: 20 }}
